...
describe Union::Operations::AddServiceCharge do
  ...
  it 'should create a service for an employee' do
    ...
    _(service_charge).wont_be_nil
    _(service_charge.charge).must_be_close_to 12.95
    _(service_charge.date).must_equal Date.new(2005, 8, 8)
    service_charge = employee.affiliation.service_charge(Date.new(2005, 8, 3))
    _(service_charge).wont_be_nil
    _(service_charge.charge).must_be_close_to 9.95
    _(service_charge.date).must_equal Date.new(2005, 8, 3)
    service_charge = employee.affiliation.service_charge(Date.new(2005, 8, 2))
    _(service_charge).must_be_nil
  end
end
